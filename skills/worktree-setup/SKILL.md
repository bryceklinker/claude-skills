...
1. **Confirm the base is clean and current.** Make sure the base branch is up to date so the worktree starts from a known-good point. Use `git.main_branch` and `git.worktree_dir` from `.craft.yml` (see `craft-code-conventions`) for the base branch and where sibling worktrees live, rather than assuming `main` and `../`.
