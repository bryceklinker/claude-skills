...
2. **Stand up the production-like environment.** Compose the app, a real database, and any external fakes (see `references/environment.md`). Use the project's `acceptance_env` and `commands.acceptance` from `.craft.yml` (see `craft-code-conventions`) — the `up`/`down` commands, the database engine, the external fakes — rather than inventing them. Apply real migrations. Confirm the app is reachable through its real interface.
