...
- **As a pre-commit gate:** scan the diff against the core rules and the smell list, and run the project's `commands.lint` and `commands.format_check` from `.craft.yml` (see `craft-code-conventions`) — the linter enforces mechanically what this page enforces by judgment. A commit that adds a comment-that-explains-what, a mutable-by-default structure, a thrown exception for an expected failure, or a method over ~10 lines isn't ready.
