...
answers, the **mechanics** as small behavior-preserving steps, and a **craft-code note**
...
**Craft-Code note:** the workhorse. Aim for functions that do one thing at one level of abstraction; the caller then reads as a short list of named steps.
...
**Craft-Code note:** use it to undo speculative indirection before re-extracting along better lines.
...
**Craft-Code note:** prefer explaining an expression by naming it over commenting it.
...
**Craft-Code note:** turns local state into a pure, independently testable function — a small win against mutable state.
...
**Craft-Code note:** a reassigned temp almost always means two values sharing one name; separate them and both become immutable.
...
**Craft-Code note:** the input-object-plus-handler shape (see `code-style/patterns.md`) — the honest home for behavior that a plain function couldn't hold.
...
**Craft-Code note:** only behavior-preserving if the tests truly pin the behavior — verify coverage before you swap.
...
**Craft-Code note:** "tell, don't ask" — behavior belongs with the data it works on.
...
**Craft-Code note:** decompose into focused operations, not a slightly smaller grab-bag.
...
**Craft-Code note:** the direct fix for message chains — callers talk only to immediate collaborators.
...
**What:** give an unexplained literal a name. **Answers:** magic number/string. **Mechanics:** declare a named constant; replace each occurrence *that means the same thing*; run tests. **Craft-Code note:** don't merge two literals that happen to share a value but not a meaning.

### Encapsulate Variable / Field
**What:** route access to a piece of data through functions. **Answers:** exposed mutable data, a field about to grow behavior. **Mechanics:** wrap reads/writes in accessors; redirect callers; run tests. **Craft-Code note:** a stepping stone — once encapsulated, it's easy to move behavior onto the data or make it immutable.
...
**Craft-Code note:** immutable value objects with behavior are the goal — this is how primitives grow up.
...
**Craft-Code note:** pairs with Replace Conditional with Polymorphism below.
...
**What:** extract the condition, then-branch, and else-branch into named functions. **Answers:** sprawling/nested conditional whose intent is buried. **Mechanics:** Extract Function on the test (`isEligibleForDiscount(...)`) and on each branch body; run tests. **Craft-Code note:** the condition and branches become one-line, intention-revealing calls.
...
**Craft-Code note:** each branch becomes a single-line return — the house-style shape.
...
**Craft-Code note:** prefer polymorphism over repeated type-branching; a single switch inside a factory is fine.
...
**Craft-Code note:** a `code-style` fix (see `architecture.md`) — null checks become rare because nulls become rare.
...
**What:** change a name to reveal intent. **Answers:** unclear name, a comment compensating for one. **Mechanics:** rename at the declaration and all uses (prefer the tool's rename); run tests. **Craft-Code note:** the cheapest, highest-leverage refactoring — nouns for variables, verbs for functions, `is/has/can` for booleans.
...
**Craft-Code note:** the group usually names a real concept (`DateRange`, `Money`) that then attracts behavior.

### Preserve Whole Object
**What:** pass the whole object instead of several values pulled off it. **Answers:** long parameter list built by unpacking one object. **Mechanics:** pass the object; let the callee extract what it needs; run tests. **Craft-Code note:** shortens call sites and often reveals the method belongs on that object (Move Function).
...
**What:** split a function that both returns a value and causes a side effect. **Answers:** a "getter" that mutates — a correctness hazard. **Mechanics:** create a pure query and a separate modifier; update callers; run tests. **Craft-Code note:** supports the read/write separation the suite favors — a query never mutates.
...
**Craft-Code note:** favor composition over inheritance when the "is-a" is really "has-a behavior."
