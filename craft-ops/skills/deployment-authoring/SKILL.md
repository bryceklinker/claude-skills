...
- **Defers the production loop to craft-code** — named generically so this skill degrades gracefully without craft-code installed: `strict-tdd` for gate/promotion/flag logic, `verification` for the declarative rollout config, `code-style` and `self-review` for how it's written and checked.
