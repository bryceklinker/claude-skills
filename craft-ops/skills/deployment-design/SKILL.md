...
- **Designing a whole rollout** (a new release path, or a redesign): decide all ten — they're all in play.
...
- **Dependency & integration readiness** — *only when the change introduces or swaps an external dependency* (a new provider, API, queue, or datastore): its capacity and rate-limit headroom for full production volume at each rollout step, and its failure mode when it degrades — fail-open vs. fail-closed, timeouts, and fallback. A new integration that's never been driven at 100% of production load, or whose behavior on a dependency outage is undefined, is an outage waiting for the widen step — the rollout plan has to know both before it advances. For a change with no new or changed dependency, discharge this in the one-line not-implicated note. Ties to blast-radius control and health-gated abort (`PRINCIPLES.md` Deployment 6 & 4).
