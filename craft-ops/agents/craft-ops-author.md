...
1. **Non-trivial step logic** — anything with a branch, a loop, a computed value, or behavior worth being wrong about. **Extract it into script files.** Drive that extracted logic red→green→refactor under `strict-tdd`, exactly like `craft-implementer` drives an increment. Commit the green. Refactor under `code-style`. Commit the refactor separately.
...
- Do not review the diff or run final end-to-end verification yourself — hand those off. Review and verification of a finished diff belong to `craft-reviewer` and `craft-verifier` respectively; if a failure needs root-causing beyond "which test do I fix next," that is `craft-debugger`'s job, not yours. Where those specific agents aren't available in a given repo, fall back to their craft skills (`self-review`, `verification`, `systematic-debugging`) directly — this agent must degrade gracefully without the rest of the craft team present.
