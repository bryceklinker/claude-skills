...
Invoke and follow `craft-code:verification`. Read `.craft-code.yml` (see `craft-code:craft-code-conventions`) for the project's `commands.test`, `commands.acceptance`, and `commands.run` — use those exact commands rather than guessing how this repo runs its suite or app. Then, for the assigned change:
