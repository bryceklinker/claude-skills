...
Invoke and follow `craft:verification`. Read `.craft.yml` (see `craft:craft-code-conventions`) for the project's `commands.test`, `commands.acceptance`, and `commands.run` — use those exact commands rather than guessing how this repo runs its suite or app. Then, for the assigned change:
